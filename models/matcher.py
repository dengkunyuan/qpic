...
# 针对ACIL，增加了HungarianMatcherHOI4IL类
class HungarianMatcherHOI4IL(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, cost_verb_class):
        bs, num_queries = outputs['pred_obj_logits'].shape[:2]
        ...
        # 虽然在计算成本矩阵时处理了整个批次的数据，但匈牙利算法仍然是在每个样本内部独立应用的，以找到每个样本的最优匹配。
        sizes = [len(v['obj_labels']) for v in targets]
        indices = [linear_sum_assignment(c[i]) for i, c in enumerate(C.split(sizes, -1))]
        return [(torch.as_tensor(i, dtype=torch.int64), torch.as_tensor(j, dtype=torch.int64)) for i, j in indices]

# 针对ACIL，进行了一些修改
def build_matcher(args, IL=False):
    if args.hoi:
        if not IL:
            return HungarianMatcherHOI(cost_obj_class=args.set_cost_obj_class, cost_verb_class=args.set_cost_verb_class,
                                       cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou)
        else:
            return HungarianMatcherHOI4IL(cost_obj_class=args.set_cost_obj_class, cost_verb_class=args.set_cost_verb_class,
                                          cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou)
    else:
        return HungarianMatcher(cost_class=args.set_cost_class, cost_bbox=args.set_cost_bbox, cost_giou=args.set_cost_giou)
